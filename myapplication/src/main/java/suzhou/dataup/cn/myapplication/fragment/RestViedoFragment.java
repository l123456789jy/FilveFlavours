...
import suzhou.dataup.cn.myapplication.adputer.VideoResoutAdputer;
 ...
 * {@link RestViedoFragment.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link RestViedoFragment#newInstance} factory method to
 * create an instance of this fragment.
 * 休息视频的界面
 */
public class RestViedoFragment extends BaseFragment implements LodeMoreCallBack {
    ...
    VideoResoutAdputer mMyadputer;
    List<VideoBean.ResultsEntity> mResultsEntityList = new ArrayList<>();
    ...
    protected void initContent() {
        ...
        mMyadputer = new VideoResoutAdputer(mResultsEntityList, options_base, mLayoutUtil);
    ...
    private void getData(int index) {
        OkHttpClientManager.get(CountUri.BASE_URI + "/休息视频/20/" + index + "", new MyHttpCallBcak() {
            @Override
            ...
            public void onResponse(final Response response) {
                try {
                    if (response != null) {
                        VideoBean homeResoutBean = null;
                        try {
                            homeResoutBean = mGson.fromJson(response.body().string(), VideoBean.class);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        List<VideoBean.ResultsEntity> results = homeResoutBean.results;
                        for (VideoBean.ResultsEntity result : results) {
